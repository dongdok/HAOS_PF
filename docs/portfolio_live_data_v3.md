...
### 1-3. 운영 품질 수치 (실측)
- 이벤트 로그 총량: **6,057건**
- 결과 품질: `success 6,054 / cancelled 3 / failed 0`
- 자동화 액션: **1,423건** (수동 액션 **216건**과 분리 추적)
- 의미: 통신/프로토콜 레이어 변경(재등록, 채널 변경) 중에도 운영 로그 정합성이 유지됨
...
- 단순 자동화 구현을 넘어, **프로토콜 선택(로컬/클라우드), 엔티티 표준화, 재등록 복구 절차**까지 운영 체계로 설계했다.
- “작동한다”가 아니라, 이벤트/후보/피드백/KPI를 DB에 남겨 **정량적으로 증명 가능한 운영**을 구현했다.
