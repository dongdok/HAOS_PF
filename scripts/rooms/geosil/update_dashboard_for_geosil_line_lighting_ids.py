...
def walk(node):
    if isinstance(node, dict):
        ...
        if entity_id in MAPPING:
            node["entity"] = MAPPING[entity_id]
        entities = node.get("entities")
        if isinstance(entities, list):
            for i, item in enumerate(entities):
                if isinstance(item, str) and item in MAPPING:
                    entities[i] = MAPPING[item]
                elif isinstance(item, dict) and item.get("entity") in MAPPING:
                    item["entity"] = MAPPING[item["entity"]]
...
async def main():
    ...
    async with websockets.connect(ws_url) as ws:
        ...
        Path("lovelace_pre_geosil_line_lighting_update.json").write_text(
        ...
        print("Updated dashboard geosil line-lighting references.")
